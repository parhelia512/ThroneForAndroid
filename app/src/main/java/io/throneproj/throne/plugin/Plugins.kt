package io.throneproj.throne.plugin
...
object Plugins {
    const val AUTHORITIES_PREFIX_SEKAI_EXE = "io.throneproj.throne.plugin."
    const val AUTHORITIES_PREFIX_NEKO_EXE = "moe.matsuri.exe."

    const val ACTION_NATIVE_PLUGIN = "io.throneproj.throne.plugin.ACTION_NATIVE_PLUGIN"

    const val METADATA_KEY_ID = "io.throneproj.throne.plugin.id"
    const val METADATA_KEY_EXECUTABLE_PATH = "io.throneproj.throne.plugin.executable_path"
    ...
    private fun getExtPluginOld(pluginId: String): List<ProviderInfo> {
        ...
        val list1 = SagerNet.application.packageManager.queryIntentContentProviders(
            Intent(ACTION_NATIVE_PLUGIN, buildUri(pluginId, "io.throneproj.throne")), flags
        )
